...
RUN cd /ctp2 \
    && ./autogen.sh && \
    CFLAGS="$CFLAGS -Wno-misleading-indentation $( [ "${BTYP##*debug*}" ] && echo -O0 || echo -ggdb -rdynamic ) -fuse-ld=gold" \
    CXXFLAGS="$CXXFLAGS -Wno-misleading-indentation -fpermissive $( [ "${BTYP##*debug*}" ] && echo -O0 || echo -ggdb -rdynamic ) -fuse-ld=gold" \
